...
static int save_ilitek_tp_info(void)
{
	char buf[80];
	
	if (ilits->tp_module == 0) {
		sprintf(buf, "hlt-k580-ili9882h-v0x%02x.%02x",
			 ilits->fw_info[50],ilits->fw_info[51]);
	} else if(ilits->tp_module == 1) {
		sprintf(buf, "tcl-k580-ili9882h-v0x%02x.%02x",
			 ilits->fw_info[50],ilits->fw_info[51]);	
	} else {
		sprintf(buf, "unknow-k580-ili9882-v0x00");
	}
	store_tp_info(buf);
	return 0;
}
#endif /*CONFIG_DEV_INFO*/
...
static int ilitek_get_tp_module(void)
{
	...
	lcd_id = of_get_named_gpio_flags(dev_node, "tn,lcd-id-gpio", 0, &flag);
	gpio_direction_input(lcd_id);

	ret = gpio_get_value(lcd_id);
	ILI_ERR("ilitek_get_tp_module ret = %d\n" ,ret);
