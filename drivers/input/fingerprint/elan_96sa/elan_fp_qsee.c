...
static int elan_read_key_status(void){
    ...
    */
    ...
    return (key_status == HW_EVENT_DOWN ? HW_EVENT_DOWN : HW_EVENT_UP);
}
...
//			}
		...
		case IOCTL_WRITE_KEY_STATUS:
            ...
            fp_event_record(key_status == 1 ? HW_EVENT_DOWN : HW_EVENT_UP);
...
static unsigned int efsa120s_poll(struct file *file, poll_table *wait)
{
    ...
    ELAN_DEBUG("%s()\n", __func__);
	__pm_relax(&fp->hal_wake_lock);
    ret = wait_event_interruptible(fp->efsa_wait, elan_work_flag > 0);
     if (ret != 0) {
...
{	
	...
	ELAN_DEBUG("reset gpio_request ret = %d\n", ret);	
	if (false/*ret < 0*/) 
	{
...
extern int store_fp_info(const char *const str);
static int save_elnan_fp_info(void)
{
	char buf[80];

	sprintf(buf, "elan_fp-7.05H-7.05T-eFSA688RA-SIP"); // ca version7.05h ta version 7.05T
	store_fp_info(buf);
	return 0;
}
#endif
