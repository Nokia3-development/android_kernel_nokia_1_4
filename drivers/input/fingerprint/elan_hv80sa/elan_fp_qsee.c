...
#include <linux/tinno_project_info.h>//add by lijian for MBA_HDW_FingerPrint_REQC6459
...
static unsigned int efsa120s_poll(struct file *file, poll_table *wait)
{
    ...
    wake_unlock(&fp->hal_wake_lock); // for 6901
...
static int elan_parse_dt(struct device *dev, struct efsa120s_data *pdata)
{
	...
	//add by lijian for get fp chip type start
	bool fp_chip_type = false;
        fp_chip_type = of_property_read_bool(np,"fp-chip-elan96sa");
        ELAN_DEBUG("lijian---fp-chip-elan96/hv80sa=%d\n",fp_chip_type);
        if(!fp_chip_type){
            printk("lijian:It is not elan96/hv80sa chip,return\n");
            return -ENODEV;
        }
        //add by lijian end
	...
	unsigned long val, void *data)
{
	...
	ELAN_DEBUG("=====%s() Start=====\n", __func__);
	ELAN_DEBUG("T: elan_fb_state_chg_callback FP\n");
...
//MBA add by lijian start for MBA_HDW_FingerPrint_REQC6459
static int tinno_get_FP_is_open(void)
{
    ...
    {
        printk(KERN_INFO "get_elanfp_hv80sa_is_open not MBA, as default open \n");
        return 1;
    }
    printk(KERN_INFO "get_elanfp_hv80sa_is_open :sensor_info = %s \n",tinno_sensor_info);

    if (NULL != strstr(tinno_sensor_info, "F"))
	return 1;
    else
	return 0;
}
//T MBA add by lijian end for MBA_HDW_FingerPrint_REQC6459
...
{	
   ...
   // struct input_dev *input_dev = NULL;
    ...
    if(!tinno_get_FP_is_open())        //MBA add by lijian for MBA_HDW_FingerPrint_REQC6459
	return -1;
