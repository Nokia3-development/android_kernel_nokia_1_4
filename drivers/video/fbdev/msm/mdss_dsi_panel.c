...
#if defined(CONFIG_PROJECT_LCM_ENN_ENP_PWR)
	if(!(memcmp("ft8006s", &pinfo->panel_name[0], 7))){

            //pr_debug("%s: %d, &pinfo->panel_name[0] =%s \n",__func__,__LINE__,&pinfo->panel_name[0]);
            ocp2132_lcd_resume(0x12);  // 5.8v
            rc = gpio_direction_output(ctrl_pdata->tp_rst_gpio,2);
            if (rc) {
