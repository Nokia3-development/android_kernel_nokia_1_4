...
#if defined(CONFIG_PROJECT_LCM_ENN_ENP_PWR)
	#if 0
		ctrl_pdata->lcd_vsn_gpio = of_get_named_gpio(
					ctrl_pdev->dev.of_node,
					"tn,lcd-vsn-gpio", 0);
		if (!gpio_is_valid(ctrl_pdata->lcd_vsn_gpio))
			pr_info("%s:%d, lcd vsn gpio not specified\n",
							__func__, __LINE__);

		ctrl_pdata->lcd_vsp_gpio = of_get_named_gpio(
					ctrl_pdev->dev.of_node,
					"tn,lcd-vsp-gpio", 0);
		if (!gpio_is_valid(ctrl_pdata->lcd_vsp_gpio))
			pr_info("%s:%d, lcd vsp gpio not specified\n",
							__func__, __LINE__);
	#endif 
		ctrl_pdata->tp_rst_gpio = of_get_named_gpio(
					ctrl_pdev->dev.of_node,
					"tn,tp-rst-gpio", 0);
		if (!gpio_is_valid(ctrl_pdata->tp_rst_gpio))
			pr_info("%s:%d, tp rst gpio not specified\n",
	     						__func__, __LINE__);

#endif /*CONFIG_PROJECT_LCM_ENN_ENP_PWR*/
