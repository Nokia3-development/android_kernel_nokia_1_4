...
extern int bq24157_charge_done;
#endif
...
#ifdef CONFIG_CHARGER_BQ2415X
	if (is_battery_charging(chip) && bq24157_charge_done >= 1 && soc_final == 99) {
		soc_final = 100;
		pr_err("battery_charging && bq24157_charge_done >= 1, so soc -> 100");
	}
#endif
...
static void check_eoc_condition(struct qpnp_bms_chip *chip)
{
	...
	if (chip->last_soc >= (75 - 1)) {
		if (status == POWER_SUPPLY_STATUS_CHARGING) {
			rc = report_eoc(chip);
			if (!rc)
				pr_debug("Disable charge for factory mini version\n");
			else
...
#ifdef FT_BUILD_FACTORY
	chip->dt.cfg_soc_resume_limit = 40;
#endif
