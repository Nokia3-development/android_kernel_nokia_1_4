...
extern struct work_struct bq2415x_setup_work;
extern struct work_struct bq2415x_set_mode_work;
extern int bq2415x_regulation_voltage;
extern int bq2415x_charge_current;
extern enum bq2415x_mode bq2415x_target_mode;
extern int bq2415x_in_boost;
#endif
...
#ifdef CONFIG_CHARGER_BQ2415X
	bq2415x_target_mode = !!disabled ? BQ2415X_MODE_OFF :
	                          BQ2415X_MODE_DEDICATED_CHARGER;
	schedule_work(&bq2415x_set_mode_work);
#endif
...
#ifdef CONFIG_CHARGER_BQ2415X
	if (bq2415x_regulation_voltage != voltage) {
		bq2415x_regulation_voltage = voltage;
		pr_err("qpnp_lbc_vddmax_set mV=%d\n", bq2415x_regulation_voltage);
...
#ifdef CONFIG_CHARGER_BQ2415X
	if (chg_current > 200 && (bq2415x_charge_current != (chg_current - QPNP_LBC_IBATMAX_MIN))) {
		bq2415x_charge_current = chg_current - QPNP_LBC_IBATMAX_MIN;
		pr_err("qpnp_lbc_ibatmax_set mA=%d\n", bq2415x_charge_current);
		schedule_work(&bq2415x_setup_work);
	}
	reg_val = (QPNP_LBC_IBATMAX_MIN - QPNP_LBC_IBATMAX_MIN) / QPNP_LBC_I_STEP_MA;
#endif
...
static void qpnp_lbc_jeita_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	if (state == ADC_TM_WARM_STATE) {
		if (temp >= chip->cfg_hot_bat_decidegc) {
			...
			chip->adc_param.low_temp =
				chip->cfg_hot_bat_decidegc - HYSTERISIS_DECIDEGC;
	...
	pr_debug("cold %d, hot %d\n", bat_cold, bat_hot);
	if (chip->bat_is_cold ^ bat_cold || chip->bat_is_hot ^ bat_hot) {
		chip->bat_is_cold = bat_cold;
		chip->bat_is_hot = bat_hot;
		if (bat_cold || bat_hot) {
			/* disable charge for THERAML */
			pr_debug("qpnp_lbc_charger_enable THERMAL 0\n");
			qpnp_lbc_charger_enable(chip, THERMAL, 0);
		} else {
			/* enable charge for THERMAL */
			pr_debug("qpnp_lbc_charger_enable THERMAL 1\n");
...
static int qpnp_charger_read_dt_props(struct qpnp_lbc_chip *chip)
{
...
#ifdef FT_BUILD_FACTORY
	chip->cfg_disable_vbatdet_based_recharge = true;
#endif
	...
	pr_debug("warm-bat-decidegc=%d, cool-bat-decidegc=%d, batt-hot-percentage=%d, batt-cold-percentage=%d\n",
			...
			chip->cfg_cold_batt_p);
#ifdef CONFIG_QPNP_SOFT_JEITA
	pr_debug("hot-bat-decidegc=%d, cold-bat-decidegc=%d\n",
...
#ifdef CONFIG_CHARGER_BQ2415X
	if (bq2415x_in_boost) {
		pr_err("usbin-valid triggered by bq2415x boost\n");
		return IRQ_HANDLED;
	}
#endif
