...
struct bq2415x_device *bq2415x_dev = NULL;
struct wakeup_source bq2415x_timer_wakelock;
struct wakeup_source bq2415x_setup_wakelock;
struct wakeup_source bq2415x_set_mode_wakelock;
struct work_struct bq2415x_setup_work;
struct work_struct bq2415x_set_mode_work;
int bq2415x_regulation_voltage = 0;
int bq2415x_charge_current = 0;
enum bq2415x_mode bq2415x_work_mode = BQ2415X_MODE_OFF;
enum bq2415x_mode bq2415x_target_mode = BQ2415X_MODE_OFF;
int bq2415x_in_boost = false;
int bq24157_charge_done = 0;
...
static int bq2415x_set_defaults(struct bq2415x_device *bq)
{
	...
	bq2415x_i2c_write(bq, BQ2415X_REG_SPECIAL, 0x04);
...
static void bq2415x_timer_work(struct work_struct *work)
{
	...
	__pm_stay_awake(&bq2415x_timer_wakelock);
	...
	if (charge < 0) {
		bq2415x_timer_error(bq, "Unknown error charge status");
		goto timer_work_end;
	} else if (charge == 2) { /* Charge done */
		bq24157_charge_done++;
	} else {
		bq24157_charge_done = 0;
	}
	pr_err("bq24157_charge_done is %d\n", bq24157_charge_done);
	...
	pr_err("bq2415x_timer_work print regs:\n");
	pr_err("%#.2x: %#.2x\n", BQ2415X_REG_STATUS,
		bq2415x_i2c_read(bq, BQ2415X_REG_STATUS));
	pr_err("%#.2x: %#.2x\n", BQ2415X_REG_CONTROL,
		bq2415x_i2c_read(bq, BQ2415X_REG_CONTROL));
	pr_err("%#.2x: %#.2x\n", BQ2415X_REG_VOLTAGE,
		bq2415x_i2c_read(bq, BQ2415X_REG_VOLTAGE));
	pr_err("%#.2x: %#.2x\n", BQ2415X_REG_VENDER,
		bq2415x_i2c_read(bq, BQ2415X_REG_VENDER));
	pr_err("%#.2x: %#.2x\n", BQ2415X_REG_CURRENT,
		bq2415x_i2c_read(bq, BQ2415X_REG_CURRENT));
	pr_err("%#.2x: %#.2x\n", BQ2415X_REG_SPECIAL,
		bq2415x_i2c_read(bq, BQ2415X_REG_SPECIAL));
	pr_err("%#.2x: %#.2x\n", BQ2415X_REG_SAFETY,
		bq2415x_i2c_read(bq, BQ2415X_REG_SAFETY));

timer_work_end:
	if (bq->autotimer)
		schedule_delayed_work(&bq->work, BQ2415X_TIMER_TIMEOUT * HZ);

	__pm_relax(&bq2415x_timer_wakelock);
}

static void bq2415x_setup_work_func(struct work_struct *work)
{
	__pm_stay_awake(&bq2415x_setup_wakelock);

	pr_err("bq2415x_setup_work_func %d mA %d mV\n",
	        bq2415x_charge_current, bq2415x_regulation_voltage);
	bq2415x_set_current_limit(bq2415x_dev, bq2415x_charge_current);
	bq2415x_set_charge_current(bq2415x_dev, bq2415x_charge_current);
	bq2415x_set_battery_regulation_voltage(bq2415x_dev, bq2415x_regulation_voltage);

	__pm_relax(&bq2415x_setup_wakelock);
}

static void bq2415x_set_mode_work_func(struct work_struct *work)
{
	pr_err("bq2415x_set_mode_work_func work %d, target %d boost %d\n",
		bq2415x_work_mode, bq2415x_target_mode, bq2415x_in_boost);

	if (bq2415x_in_boost)
		return;
	if (bq2415x_work_mode == bq2415x_target_mode)
		return;

	bq24157_charge_done = 0;

	__pm_stay_awake(&bq2415x_set_mode_wakelock);

	bq2415x_set_mode(bq2415x_dev, bq2415x_target_mode);
	if (bq2415x_work_mode == BQ2415X_MODE_OFF) {
		bq2415x_set_autotimer(bq2415x_dev, 0);
	} else if (bq2415x_work_mode == BQ2415X_MODE_BOOST) {
		bq2415x_set_autotimer(bq2415x_dev, 1);
	} else {
		bq2415x_set_autotimer(bq2415x_dev, 1);
		schedule_work(&bq2415x_setup_work);
	}

	__pm_relax(&bq2415x_set_mode_wakelock);

	pr_err("bq2415x_set_mode_work_func end %d\n", bq2415x_work_mode);
}
					    ...
					    char *buf)
{
	...
	ret += bq2415x_sysfs_print_reg(bq, BQ2415X_REG_SPECIAL, buf+ret);
	ret += bq2415x_sysfs_print_reg(bq, BQ2415X_REG_SAFETY, buf+ret);
	return ret;
}
...
struct bq2415x_otg_regulator_data {
	struct regulator_desc	rdesc;
	struct regulator_dev	*rdev;
};
static struct bq2415x_otg_regulator_data bq2415x_otg_regulator;

static int bq2415x_otg_regulator_enable(struct regulator_dev *rdev)
{
	__pm_stay_awake(&bq2415x_set_mode_wakelock);

	pr_err("bq2415x_otg_regulator_enable");
	bq2415x_in_boost = true;
	bq2415x_set_mode(bq2415x_dev, BQ2415X_MODE_BOOST);
...
static int bq2415x_otg_regulator_disable(struct regulator_dev *rdev)
{
	...
	pr_err("bq2415x_otg_regulator_disable");
	bq2415x_in_boost = false;
	bq2415x_set_mode(bq2415x_dev, BQ2415X_MODE_OFF);
...
static int bq2415x_otg_regulator_is_enable(struct regulator_dev *rdev)
{
	...
	command = BQ2415X_BOOST_STATUS;
	ret = bq2415x_exec_command(bq2415x_dev, command);
	if (ret < 0) {
		pr_err("failed to read bq2415x OTG status!\n" );
		return  0;
	}

	if (!!ret) {
		pr_debug("bq2415x OTG on\n" );
		return 1;
	} else {
		pr_debug("bq2415x OTG off\n" );
		return 0;
	}
}

static struct regulator_ops bq2415x_otg_reg_ops = {
	.enable		= bq2415x_otg_regulator_enable,
	.disable	= bq2415x_otg_regulator_disable,
	.is_enabled	= bq2415x_otg_regulator_is_enable,
};

static int bq2415x_otg_regulator_init(struct i2c_client *client)
{
	int rc = 0;
	...
	init_data = of_get_regulator_init_data(&client->dev,
		client->dev.of_node, &bq2415x_otg_regulator.rdesc);
	if (!init_data) {
	...
	if (init_data->constraints.name) {
		bq2415x_otg_regulator.rdesc.owner = THIS_MODULE;
		bq2415x_otg_regulator.rdesc.type = REGULATOR_VOLTAGE;
		bq2415x_otg_regulator.rdesc.ops = &bq2415x_otg_reg_ops;
		bq2415x_otg_regulator.rdesc.name = init_data->constraints.name;
		...
		bq2415x_otg_regulator.rdev = regulator_register(
				&bq2415x_otg_regulator.rdesc, &cfg);
		if (IS_ERR(bq2415x_otg_regulator.rdev)) {
			rc = PTR_ERR(bq2415x_otg_regulator.rdev);
			bq2415x_otg_regulator.rdev = NULL;
			if (rc != -EPROBE_DEFER)
				dev_err(&client->dev,
					"failed to register bq2415x_otg_regulator, rc=%d\n", rc);
		}
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	bq2415x_dev = bq;
	ret = bq2415x_otg_regulator_init(client);
	if (ret) {
		dev_err(bq->dev, "failed to init bq2415x otg regulator: %d\n", ret);
		goto error_4;
	}
	INIT_WORK(&bq2415x_setup_work, bq2415x_setup_work_func);
	INIT_WORK(&bq2415x_set_mode_work, bq2415x_set_mode_work_func);
	wakeup_source_init(&bq2415x_timer_wakelock, "bq2415x_timer_wakelock");
	wakeup_source_init(&bq2415x_setup_wakelock, "bq2415x_setup_wakelock");
	wakeup_source_init(&bq2415x_set_mode_wakelock, "bq2415x_set_mode_wakelock");
	bq2415x_regulation_voltage = bq->init_data.battery_regulation_voltage;
	bq2415x_charge_current =  bq->init_data.charge_current;
	bq2415x_work_mode = BQ2415X_MODE_DEDICATED_CHARGER;
	bq2415x_target_mode = BQ2415X_MODE_DEDICATED_CHARGER;
	bq2415x_in_boost = false;
...
static int bq2415x_remove(struct i2c_client *client)
{
	...
	wakeup_source_trash(&bq2415x_timer_wakelock);
	wakeup_source_trash(&bq2415x_setup_wakelock);
	wakeup_source_trash(&bq2415x_set_mode_wakelock);

	regulator_unregister(bq2415x_otg_regulator.rdev);
	bq2415x_dev = NULL;
