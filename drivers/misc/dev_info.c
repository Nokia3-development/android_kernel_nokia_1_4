...
static char info_fingerprint[INFO_LEN];
...
/*int printf_main_camera_info(const char *const str)
{
        CDBG("%s : str=%s\n", __func__, str);
        return 0;
}
EXPORT_SYMBOL_GPL(printf_main_camera_info);*/
...
int store_fp_info(const char *const str)
{
	CDBG("%s : str=%s\n", __func__, str);
	return snprintf(info_fingerprint, INFO_LEN, "%s", str);
}
EXPORT_SYMBOL_GPL(store_fp_info);

static ssize_t fp_info_show(struct kobject *kobj, struct kobj_attribute *attr,
		      char *buf)
{
	CDBG("%s : fp_info=%s\n", __func__, info_fingerprint);
	return snprintf(buf, INFO_LEN, "%s\n", info_fingerprint);
}
...
static struct kobj_attribute info_fingerprint_attribute =
	__ATTR(info_fingerprint, 0664, fp_info_show, NULL);
...
static struct attribute *attrs[] = {
	...
	&info_fingerprint_attribute.attr,
	NULL,
};
...
//Ramiel add p7203 camera  ---------------------
   ...
   else if(!strncmp(sensor_name, "s5k3p9ps_ofilm_x800as", sizeof("s5k3p9ps_ofilm_x800as")))
   {
	...
	else if(!strncmp(sensor_name, "s5k4h7", sizeof("s5k4h7")))
	{//pr_err("%s : sensor_name=%s\n", __func__, sensor_name);
		 rc = snprintf(main_camera_and_eeprom, DEV_INFO_LEN, "%s_(8M)", "s5k4h7_k580");
		 if(rc < 0){
			 store_main_camera_info(sensor_name);
		 }else{
			 store_main_camera_info(main_camera_and_eeprom);
	...
	else if(!strncmp(sensor_name, "gc8034", sizeof("gc8034")))
	{
		 rc = snprintf(main_camera_and_eeprom, DEV_INFO_LEN, "%s_(8M)", "gc8034_k580");
		 if(rc < 0){
			 store_main_camera_info(sensor_name);
		 }else{
			 store_main_camera_info(main_camera_and_eeprom);
	...
	else if(!strncmp(sensor_name, "gc5035", sizeof("gc5035")))
	{
		 rc = snprintf(sub_camera_and_eeprom, DEV_INFO_LEN, "%s_(5M)", "gc5035_k580");
		 if(rc < 0){
			 store_sub_camera_info(sensor_name);
		 }else{
			 store_sub_camera_info(sub_camera_and_eeprom);
	...
	else if(!strncmp(sensor_name, "s5k5e9", sizeof("s5k5e9")))
	{//pr_err("%s : sensor_name=%s\n", __func__, sensor_name);
		 rc = snprintf(sub_camera_and_eeprom, DEV_INFO_LEN, "%s_(5M)", "s5k5e9_k580");
		 if(rc < 0){
			 store_sub_camera_info(sensor_name);
		 }else{
			 store_sub_camera_info(sub_camera_and_eeprom);
	...
	#if defined(CONFIG_CAMERA_AUX_INFO)
    else if(!strncmp(sensor_name, "gc02m1", sizeof("gc02m1")))
	{
		 rc = snprintf(aux_camera_and_eeprom, DEV_INFO_LEN, "%s_(2M)", "gc02m1_k580");
		 if(rc < 0){
			 store_aux_camera_info(sensor_name);
		 }else{
			 store_aux_camera_info(aux_camera_and_eeprom);
	...
	else if(!strncmp(sensor_name, "ov02b10", sizeof("ov02b10")))
	{//pr_err("%s : sensor_name=%s\n", __func__, sensor_name);
		 rc = snprintf(aux_camera_and_eeprom, DEV_INFO_LEN, "%s_(2M)", "ov02b10_k580");
		 if(rc < 0){
			 store_aux_camera_info(sensor_name);
		 }else{
			 store_aux_camera_info(aux_camera_and_eeprom);
